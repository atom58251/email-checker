...
```
Пользователь → Netlify (Next.js) → БРАУЗЕР: разбор файла, разбивка
                                            многоадресных ячеек, дедуп,
                                            офлайн-проверки, SHA-256 хэши
                                            (lib/processFile.ts)
                                  → Edge Function 'check-suppression'
                                       (батчами по 10000 хэшей → Postgres)
                                  → Edge Function 'resolve-mx'
                                       (батчами по 300 уникальных доменов
                                        → Deno.resolveDns)
...
**Многоадресные ячейки и дубли (`lib/processFile.ts`):** если в ячейке с
email несколько адресов через запятую, точку с запятой, пробел или
перенос строки — в исходной ячейке остаётся первый адрес, остальные
добавляются как новые строки **в конец списка** (остальные колонки
дублируются из исходной строки). Поэтому в итоговом xlsx строк может
быть **больше**, чем в исходном файле — это ожидаемо, не баг. Дубли
считаются только **внутри загруженного файла** (не против suppression-
list) по адресу без учёта регистра: первое вхождение проверяется как
обычно, повторные помечаются статусом `DUPLICATE_IN_FILE` и не идут
в suppression/MX-проверки.
